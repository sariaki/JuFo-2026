...
void __cdecl foo(int x)
{
	...
	printf("foo %i\n", x); // This Basic Block will always be executed.
			...
			\caption{Dieselbe Funktion aber mit PoP (Bernsteinpolynomgrad $n=3 $) obfuskiert. Pseudo-C wurde aus der Dekompilation von IDA entnommen.}
			\label{fig:doneObf}
		\end{subfigure}
	\end{minipage}
\end{figure}
...
Für den Evaluationsprozess wurde gemäß \cite{jinFrameworkQuantifyQuality2024} vorgegangen.\textcolor{red}{TODO: updaten oder nicht erwähnen!}
Es wurden dabei die Programme aus \cite{banescuCodeObfuscationSymbolic2016}\footnote{\url{https://github.com/tum-i4/obfuscation-benchmarks}} sowie die GNU Core Utilities verwendet. % sowie ausgewählte Programme aus der NIST Juliet Test Suite v.1.3 \cite{NISTJuliet}
