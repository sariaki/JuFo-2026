...
\begin{minipage}[t]{0.45\textwidth}
  ...
  \captionof{figure}{Kontrollflussgraph einer einfachen Funktion mit opakem Prädikat. Abbildung aus der Dekompilation des Spiels \emph{Overwatch} mittels IDA entnommen.} \label{fig:overwatchGraph}
\end{minipage}\hfill
\begin{minipage}[t]{0.5\textwidth}
  \centering
  % \resizebox{\columnwidth}{!}{\usebox{\idacode1}}
  \includegraphics[scale=0.2]{komplex_cropped}
  \captionof{figure}{Ausschnitt des Kontrollflussgraphen einer Funktion mit vielen opaken Prädikaten. Abbildung aus der Dekompilation des Spiels \emph{Overwatch} mittels IDA entnommen.} \label{fig:overwatchGraph2}
\end{minipage} \\
...
void foo(int x)
{
	printf("foo %i\n", x);
		...
		\end{lstlisting}
		\caption{Eine zu obfuskierende Funktion.}
		\label{fig:toObf}
	\end{subfigure}
	\hfill
...
void __cdecl foo(int x)
{
	...
	{
		printf("FalseBB says hi\n");
		__debugbreak()
	}
	printf("TrueBB says hi\n");
	printf("foo %i\n", x);
}
		\end{lstlisting}
...
Folgende Programme wurden dabei als Input verwendet: \textcolor{red}{TODO: ...}
