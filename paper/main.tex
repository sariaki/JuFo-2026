...
Die symbolischen Ausführungsengines zugrundeliegenden SMT-Solver können im Wesentlichen folgendes:
\begin{enumerate}
    \item Den Wahrheitswert einer Aussage herauszufinden.
    \item Variablen mögliche Werte zuzuweisen, damit diese Aussage erfüllt ist.
\end{enumerate}
In der Regel genügt dies für die Untersuchung deterministischer Algorithmen und entscheidbarer Probleme. Um den Wahrheitswert einer Aussage zu finden, muss sie für jeden möglichen Wert jeder Variable vom symbolische Ausführungsengine bewiesen werden. % TODO: Soll das nicht lieber in 3.3?

Will man allerdings Aussagen über einen probabilistischen Algorithmus treffen, so ist dies ohne erhebliche manuelle Eingriffe eines Nutzers in Form von extra Beschränkungen (\emph{constraints} unmöglich. Jede zufällige Variable eines jeden Schleifenaufrufes muss vom constraint solver als symbolische Variable betrachtet und somit für alle möglichen Werte überprüft werden. Bei einem Monte-Carlo-Algorithmus heißt dies allerdings, dass auch unwahrscheinliche Wariablenwerte, welche zu \emph{falschen} Ergebnissen führen, überprüft werden. Der Wahrheitsgehalt wird somit zwar formell \textit{logisch} korrekt bewiesen - praktisch allerdings nicht.
\begin{example}
...
\begin{enumerate}
    ...
    \item Lege $F(a)=0$ und $F(b)=1$ fest.
    \item Für jede Stelle $x_i\in[a;b]$ der $n$ Stellen: wähle einen Punkt $y_i$ für $F$ an dieser Stelle, sodass $y_i\in[0;1]$ maximal $\frac{1}{n}$ über $y_{i-1}$ liegt.
    \item Führe eine Regression für die Punkte durch.
\end{enumerate}
Es genügt hierbei nicht, eine einfache Polynomregression durchzuführen, da hier Oszillationen zwischen den ausgewählten Punkten $(x_i,y_i)$ entstehen können. Hierdurch wäre Bedigung 1 (vgl. \cref{def:kumVer}) nicht erfüllt. Um dieses Problem zu lösen, gibt es im Wesentlichen 2 Methoden:
\begin{enumerate}
    \item \textbf{Splines:} Regression so durchführen, dass $F$ stückweise definiert ist über mehrere kleinere Funktionen, welche alle notwendigen Bedingungen erfüllen. Hierfür gibt es mehrere mögliche Regressionsarten.
    \item \textbf{Bernstein-Polynom-Regression:} Auf dem skalierten Intervall $[0,1]$ parametrisieren wir $F$ durch ein Bernstein-Polynom
    $$
    B_n(x) \;=\; \sum_{k=0}^n c_k \binom{n}{k} x^k (1-x)^{\,n-k}.
    $$
    \begin{enumerate}[label={(\arabic*)}]
        \item Monotonie von $B_n$ folgt, wenn die Koeffizienten $c_k$ nicht fallend sind (d.\,h. $c_0\le c_1\le\cdots\le c_n$).
        \item Stetigkeit ist gegeben, da es sich um eine Funktion handelt.
        \item Da $F(a)=0$ und $F(b)=1$ ist und der Definitionsbereich von $F$ das Intervall $[0;1]$ ist, ist $\lim_{x\to -\infty} F(x) = 0 \text{ und } \lim_{x\to +\infty} F(x)=1$.
    \end{enumerate}
    Das Fitten \footnote{Finden nahe-optimaler Parameter $c_0,\dots,c_k$, sodass $B_n$ die Punkte $(x_1, y_1),\dots,(x_n, y_n)$ bestmöglich beschreibt} kann als \emph{Non-Negative Least-Squares}-Optimierungsproblem formuliert werden:
    \begin{equation}
    \begin{split}
    \min_{c\in\mathbb{R}^{n+1}} \|\mathbf{A} c - \mathbf{y}\|_2^2 \\\text{u. d. N.}\quad \mathbf{D} c \ge 0,\; 0\le c_k \le 1,
    \end{split}
    \end{equation}
wobei die Designmatrix $A_{i,k}=\binom{n}{k} x_i^k (1-x_i)^{n-k}$ und $\mathbf{D}$ die Vorwärtsdifferenz-Matrix ist.
\end{enumerate}
...
%     \begin{tabular}{lccc}
%         \hline
%         Ansatz                     & Laufzeitkosten (Programm) & Laufzeitkosten (beim Obfuskieren) & Präsizion\\
%         \hline
%         Verteilungsklasse         & & &\\
%         Splines                   & & &\\
%         Bernstein-Polynom-Regression & & &\\
%         Isotone Regression        & & &\\
%         \hline
%     \end{tabular}
%     \caption{Vergleich der Ansätze.}
%     \label{tab:vergleich_ansatze}
% \end{table}
Es wurde sich für eine Bernstein-Polynom-Regression entschieden aufgrund der effizienten Regression und den gering eintretenden Laufzeitkosten bei obfuskierten Programmen. Die Abweichung ...asd
 
% \paragraph{Generierung über Splines}
% ...
% Diese Methode hat den Nachteil, dass sie in Form von Verzweigungen in ein Programm beim obfuskieren überführt werden muss. Dies geschieht auf Kosten der Laufzeit und der obfuskierten Programmgröße.
% \paragraph{Generierung über Isotone Regression (PAV)}
% \paragraph{Generierung über Bernstein-Polynom-Regression}
