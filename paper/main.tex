...
\begin{definition}[Probabilistische opake Prädikate] \label{def:probOpaque} \label{def:konstruktionsschema}
	Sei $\mathcal{O}_p : \Phi \rightarrow \{0, 1\}$ eine Abbildung einer Variable $\phi\in \Phi$ zu einem Prädikat. Das Prädikat $\mathcal{O}_p(\phi)$ ist probabilistische opak, wenn der Fall $A\in \{0, 1\}$ mit einer so hohen Wahrscheinlichkeit eintritt, dass der Fall $\overline{A}$ vernachlässigbar ist. \textcolor{red}{TODO: formeller}
\end{definition}
...
\end{equation}
Diese hat den Vorteil, dass sich verschiedene andere Verteilungen (z.B. Chi-Quadrat-, Erlang und Exponentialverteilung) aus ihr ergeben. Ein \emph{Pattern-Matching} Angriff wird hierdurch erschwert, da man nach Termen der sehr allgemeinen Form $ax^n b^x$ suchen müsste \footnote{Die Faktoren werden im Vorhinein zur Kompillationszeit miteinander multipliziert}. 
...
void __cdecl foo(int x)
{
	...
	{
		// Junkcode
		__debugbreak() // This Basic Block is never reached.
	}
	printf("foo %i\n", x); // This Basic Block will always be executed.
}			\end{lstlisting}
			\caption{Dieselbe Funktion aber mit PoP obfuskiert. Pseudo-C wurde aus der Dekompilation von IDA entnommen.}
			\label{fig:doneObf}
		\end{subfigure}
	\end{minipage}
\end{figure}
...
Für die Evaluierung wurde sich aufgrund ihrer Verbreitung für die Kriterien von Collberg et al. \cite{collbergATaxonomyOfObfuscatingTransformations} gegenüber anderen (\textcolor{red}{...}) entschieden\label{par:metriken}:
...
Für den Evaluationsprozess wurde gemäß \cite{jinFrameworkQuantifyQuality2024} vorgegangen.
%Folgende Programme wurden entsprechend obfuskiert: \textcolor{red}{TODO: obfuscator-benchmarks, Paper mit GNU core utilities und NIST Juliet}
Es wurden dabei die Programme aus \cite{banescuCodeObfuscationSymbolic2016}\footnote{https://github.com/tum-i4/obfuscation-benchmarks}, die GNU Core Utilities sowie ausgewählte Programme aus der NIST Juliet Test Suite v.1.3 \cite{NISTJuliet} verwendet.

Alle Programme wurden mit clang 18.1.3 und Optimierungslevel \texttt{-O3} kompilliert.

Folgende symbolische Ausführungsengines wurden verwendet: Angr, Triton, Miasm \textcolor{red}{TODO: ...}
...
\paragraph{Stärke} \label{sec:stärke}
