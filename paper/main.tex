...
\end{enumerate}
Die Berechnung des Inversen der kumulativen Wahrscheinlichkeitsdichtefunktion geschieht folgendermaßen:
\begin{algorithm}[!htb] 
    \caption{Berechnung des Inversen (Binärsuche)}
    \label{alg:invCumSum}
    \begin{algorithmic}[1]
        \Require $\Var{D_{start}}, \Var{D_{end}}$ (Domain), $\Var{C}$ (Bernstein coefficients), $\Var{HShift}, \Var{HStretch}$ (Horizontal shift and stretch)
        \Function{SampleBernsteinInverse}{$\Var{U} \in [0, 1]$}
            ...
            \While{$\Var{Iter} < 64$} \Comment{Fixed iterations for double precision}
...
Ein LLVM-Pass bietet in diesem Fall das beste Kompromissverhältnis zwischen \emph{low-level} Kontrolle, Laufzeiteffizienz, einer \emph{high-level} Portabilität sowie geringem Entwicklungsaufwand. %Im Falle anderer Obfuskatoren mit architekturspezifischem Fokus mag dies anders sein.
...
\paragraph{Explizite Quellen (APIs, Zeit, Hardwaretimer)}
CPUs und Betriebssysteme bieten mehrere Methoden, welche sich für die Zufallszahlgenerierung eignen (z.B. \texttt{rdtsc} oder \texttt{rand()}). 
...
\begin{tabular}{c|l}
	...
	\hline
	Tarnung & Wie leicht lässt sich ein obfuskiertes Programm als solches erkennen? \\
\end{tabular}
\end{center}
