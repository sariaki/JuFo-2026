...
entry:
  %0 = call i32 (ptr, ...) @printf(ptr @fmt_u, double %u)
  %p0 = call double @exp(double 0x19000000000000)
  br label %loop

loop:                                             ; preds = %loop, %entry
  %k = phi i64 [ 0, %entry ], [ %1, %loop ]
  %p = phi double [ %p0, %entry ], [ %4, %loop ]
  %s = phi double [ %p0, %entry ], [ %5, %loop ]
  %1 = add i64 %k, 1
  %2 = sitofp i64 %1 to double
  %3 = fdiv double 0x19000000000000, %2
  %4 = fmul double %p, %3
  %5 = fadd double %s, %4
  %6 = fcmp ogt double %u, %s
