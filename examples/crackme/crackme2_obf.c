// https://github.com/NoraCodes/crackmes/blob/master/crackme05e.c

#include <stdio.h>
#include <string.h>
#include <stdlib.h>

#define OBFUSCATE __attribute__((annotate("POP")))

// Flag: ./crackme2 "w m o'#m;>'#C\`A@"
// Flag: ./crackme2 'A8m 8[1wX>'\''~!@A`'
OBFUSCATE void succeed(char* string)
{
    printf("Ja, %s ist richtig!\n", string);
    exit(0);
}

OBFUSCATE void fail(char* string)
{
    printf("Nein, %s ist falsch.\n", string);
    exit(1);
}

OBFUSCATE int check_with_mod(char* substring, int n, int mod)
{
    int sum = 0;
    for (int i = 0; i < n; i++)
    {
        sum = sum + substring[i];
    }
    return (sum % mod) == 0;
}

__attribute__((noinline)) OBFUSCATE void obfuscated(char* input, void* x)
{
    int len = strnlen(input, 1000);
    if (len != 16)
    {
        fail(input);
    }

    // Add some fixed characters
    if (input[2] != 'm')
    {
        fail(input);
    }

    if (input[10] != '\'')
    {
        fail(input);
    }

    // Make the actual modulo checks
    if (!check_with_mod(input, 4, 2))
    {
        fail(input);
    }

    if (!check_with_mod(input + 4, 4, 3))
    {
        fail(input);
    }

    if (!check_with_mod(input + 8, 4, 5))
    {
        fail(input);
    }

    if (!check_with_mod(input + 12, 4, 2))
    {
        fail(input);
    }

    succeed(input);
}

OBFUSCATE int main(int argc, char** argv)
{
    if (argc != 2)
    {
        printf("Zu viele Argumente! (>1).\n");
        return -1;
    }

    char* input = argv[1];

    volatile void* x = (void*)time(NULL);
    obfuscated(input, x);
}